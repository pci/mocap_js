// Mocap.js copyright Philip Ingrey 2023
...
function motionSensor(options) {
  ...
  this.init = function () {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      ...
      var errorMsg =
        "<p class=\"error\">Uh oh, it appears your browser doesn't support this feature.<br>Please try with a browser that has camera support. <br/> e.g. <a href='http://www.opera.com/'>opera</a>, <a href='http://www.google.co.uk/chrome'>chrome</a> or <a href='http://nightly.mozilla.org/'>firefox nightly</a></p>";
    ...
    if (lastimg && PAINTWIDTH != 0) {
        ...
        for (var j = 0; j < noblocks; j++) {
          ...
          for (var ii = dw * i; ii < dw * (i + 1); ii++) {
            for (var jj = dh * j; jj < dh * (j + 1); jj++) {
              ...
              sum +=
                that.abs(
                  lastimg.data[4 * (PAINTWIDTH * jj + ii)] -
                    current.data[4 * (PAINTWIDTH * jj + ii)]
                ) +
                that.abs(
                  lastimg.data[4 * (PAINTWIDTH * jj + ii) + 1] -
                    current.data[4 * (PAINTWIDTH * jj + ii) + 1]
                ) +
                that.abs(
                  lastimg.data[4 * (PAINTWIDTH * jj + ii) + 2] -
                    current.data[4 * (PAINTWIDTH * jj + ii) + 2]
                );
            }
          }

          // max of sum = 256*noblocks^2
          blocks[noblocks * i + j] = Math.min(
            255,
            (2 * sum) / (3 * noblocks * noblocks)
          );
        }
      }
      ...
      if (msum) {
        ...
        difftx.fillStyle =
          (1.0 * msum) / dmax > 1
            ? "rgb(0,255,0)"
            : "rgb(" + parseInt(Math.min((255.0 * msum) / dmax, 255)) + ",0,0)";
      ...
      PAINTHEIGHT = c.height = v.videoHeight;
      if (PAINTWIDTH > 0)
        lastimg = ctx.getImageData(0, 0, PAINTWIDTH, PAINTHEIGHT);
    }
  };
